...
class PawsRobot:
    ...
    def createTrotTrajectory(self, stepLength=0.1, stepHeight=0.05, T=2, t_0=0, sampleRate=500, dutyFactor = 0.5,
        ...
        ySwing = L1 + np.zeros(samplesPerSwing)
        yStance = L1 + np.zeros(samplesPerStance)
    ...
    def createWalkTrajectory(self, stepLength=0.1, stepHeight=0.05, T=4, t_0=0, sampleRate=500, dutyFactor=0.75,
        ...
        x_2 = x_0 + (1 + backSwingCoef) * self.stepLength
        x_3 = x_0 + self.stepLength
        x_s1 = x_3 - self.stepLength/3
        x_s2 = x_3 - 2*self.stepLength/3
        h = self.stepHeight
        ...
        xStance1 = x_3 + (20*x_s1 - 20*x_3)/(2*(T_swing - t_0)**3)*np.power(t_stance - t_0, 3) \
                  + (30*x_3 - 30*x_s1)/(2*(T_swing - t_0)**4)*np.power(t_stance - t_0, 4) \
                  + (12*x_s1 - 12*x_3)/(2*(T_swing - t_0)**5)*np.power(t_stance - t_0, 5)

        xStance2 = x_s1 + (20*x_s2 - 20*x_s1)/(2*(T_swing - t_0)**3)*np.power(t_stance - t_0, 3) \
                   + (30*x_s1 - 30*x_s2)/(2*(T_swing - t_0)**4)*np.power(t_stance - t_0, 4) \
                   + (12*x_s2 - 12*x_s1)/(2*(T_swing - t_0)**5)*np.power(t_stance - t_0, 5)

        xStance3 = x_s2 + (20*x_0 - 20*x_s2)/(2*(T_swing - t_0)**3)*np.power(t_stance - t_0, 3) \
                   + (30*x_s2 - 30*x_0)/(2*(T_swing - t_0)**4)*np.power(t_stance - t_0, 4) \
                   + (12*x_0 - 12*x_s2)/(2*(T_swing - t_0)**5)*np.power(t_stance - t_0, 5)
    ...
    def simulateTrotGate(self):
        ...
        RL_targetJointPositions = paws.calculateIK(FR_targetFootPosition, kneeLagging=False)
        ...
        RR_targetJointPositions = paws.calculateIK(FL_targetFootPosition, kneeLagging=False)
    ...
    def simulateWalkGate(self):
        ...
        RL_targetJointPositions = paws.calculateIK(RL_targetFootPosition, kneeLagging=False)
        FL_targetFootPosition = self.footPositionTrajectory[int(samplesPerPeriod/2)]
        FL_targetJointPositions = paws.calculateIK(FL_targetFootPosition, kneeLagging=True)
        RR_targetFootPosition = self.footPositionTrajectory[int(samplesPerPeriod/4)]
        RR_targetJointPositions = paws.calculateIK(RR_targetFootPosition, kneeLagging=False)
